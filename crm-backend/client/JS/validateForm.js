export const validateClientForm = () => {
  ...
  const checkRequiredName = (input, message) => {
      if (!input.value) {
          userName.style.borderColor = 'var(--color-burnt-sienna)';
          userSurname.style.borderColor = 'var(--color-burnt-sienna)';
          message.textContent = `Введите Фамилию и Имя клиента!`;
          return false;
      } else {
          message.textContent = '';
      }
  ...
  if (!checkRequiredName(userSurname, writeSurname)) { return false };
  if (!checkRequiredName(userName, writeName)) { return false };
